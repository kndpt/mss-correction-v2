...
import { generateRandomCoverLink } from 'src/utils/utils';
...
export const _metaKeywords = [
  "améliorer style d'écriture",
  "techniques d'écriture",
  'conseils écriture',
  'style littéraire',
  'rédaction facile',
...
export default function PostNewEditForm({ currentPost }: Props) {
  ...
  const { createPost, updatePost } = useFirestorePosts();
  ...
  const coverUrl = generateRandomCoverLink();
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      coverUrl: currentPost?.coverUrl || coverUrl,
  ...
  const handleChangePublish = (event: React.ChangeEvent<HTMLInputElement>) => {
    const { checked } = event.target;
    if (checked) {
      methods.setValue('publish', 'published');
    } else {
      methods.setValue('publish', 'draft');
    }
  };
        ...
        <Card>
          ...
          <Stack spacing={3} sx={{ p: 3 }}>
            ...
            <RHFAutocomplete
              ...
              options={_metaKeywords.map((option) => option)}
              getOptionLabel={(option) => option}
              renderOption={(props, option) => (
                <li {...props} key={option}>
                  {option}
  ...
  const renderActions = (
    <>
      ...
      <Grid xs={12} md={8} sx={{ display: 'flex', alignItems: 'center' }}>
        <FormControlLabel
          control={
            <Switch checked={values.publish === 'published'} onChange={handleChangePublish} />
          }
          label="Publish"
          sx={{ flexGrow: 1, pl: 3 }}
        />
